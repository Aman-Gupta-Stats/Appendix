...
def show_dashboard():
    ...
    if role == "RD":
        ...
        file_path = "Data/KYC.xlsx"  # fallback
    ...
    with complete_tab:
        ...
        try:
            ...
            # If no “Complete” sheet exists, just show the same data or show a message:
            st.info("No separate “Complete” sheet found. Displaying the same data as Pending for now.")
            try:
                st.dataframe(pd.read_excel(file_path), use_container_width=True)
            except:
                st.error(f"Unable to load data from `{file_path}`.")
        except FileNotFoundError:
            st.error(f"Could not find the file at `{file_path}`. Make sure it exists.")
        except Exception as e:
...
def show_kyc_view():
    st.title(f"KYC Automation - {st.session_state.company_name}")
    st.markdown("---")
