...
def live_user_dashboard():
    ...
    st.title(language_data.get("Welcome",{}).get(st.session_state.language,"Welcome")+" " + language_data.get("Aman",{}).get(st.session_state.language,"Aman"))
    st.markdown("---")

    tab1, tab2, tab3 = st.tabs([language_data.get("Feed",{}).get(st.session_state.language,"Feed"), language_data.get("Alumni Network",{}).get(st.session_state.language,"Alumni Network"), language_data.get("Videos",{}).get(st.session_state.language,"Videos")])
    with tab1:
...
def live_user_onboard_dashboard():
    ...
    tab1, tab2, tab3, tab4 = st.tabs([language_data.get("Profile",{}).get(st.session_state.language,"Profile"), language_data.get("Feed",{}).get(st.session_state.language,"Feed"), language_data.get("Magic Connect",{}).get(st.session_state.language,"Magic Connect"), language_data.get("Videos",{}).get(st.session_state.language,"Videos")])
    with tab1:
        cols = st.columns([4, 6])
        with cols[0]:
...
def admin_dashboard():
    ...
    st.header("Target Audience Visualization (18-25 & BPL)")
    ...
    fig_top_districts = px.bar(
        ...
        title="Top 5 districts by youth count", orientation="h"
    )
