...
def show_end_page():
    ...
    if st.session_state.role == "RD":
        align = st.columns([0.5,1])
        with align[1]:
            st.markdown("### Thanks for using RD Helper")
        align = st.columns([0.65,1])
        with align[1]:
            st.write("Redirecting to Home Page...")
        time.sleep(5)
...
def show_kyc_second_view():
    ...
    bd, od, am = st.tabs(["Basic Details", "Ownership Details", "Adverse Media"])
    with bd:
    ...
    if st.session_state.generate_report_kyc2:
        ...
        if st.session_state.generate_report_kyc2_act:
            col = st.columns([1,1])
            with col[0]:
                if st.button("Evidence", key = "ev", use_container_width=True):
                    pass

            with col[1]:
                if st.button("CAAR", key = "ca", use_container_width=True):
                    pass
    ...
    if st.session_state.compare_difference == True:
        ...
        with bd:
            ...
            with al[0]:
                ...
                bd_cn = basic_details[["Value", "Issue Criticality"]]
                ...
                basic_details_edit = st.data_editor(basic_details_styled, row_height = 56, height = 598,
                                                    column_config = {
                ...
                with al[1]:
                    ...
                    with st.popover("Upload Evidence"):
                        evidence_8 = st.file_uploader("Upload Evidence", type="JPEG",accept_multiple_files = True,key = "evidence_8")

                    with st.popover("Upload Evidence"):
                        evidence_9 = st.file_uploader("Upload Evidence", type="JPEG",accept_multiple_files = True,key = "evidence_9")

                    with st.popover("Upload Evidence"):
                        evidence_10 = st.file_uploader("Upload Evidence", type="JPEG",accept_multiple_files = True,key = "evidence_10")
            ...
            with cols[0]: 
                ...
                if st.button("Save Progress", use_container_width=True, key = "bd_sp"):
                    # bd_list = basic_details_edit[basic_details_edit["Ask Client?"] == True].Value.tolist()
                    pass
        ...
        with od:
            ...
            with cl[0]:
                ...
                od_cn = owner_details[["Value", "Issue Criticality"]]
            ...
            with cols[0]:           
                if st.button("Save Progress", use_container_width=True, key = "od_sp"):
                    # od_list = owner_details_edit[owner_details_edit["Ask Client?"] == True].Value.tolist()
                    pass
        ...
        with tr:
            final_list = ["CAAR"] + basic_details_edit[basic_details_edit["Ask Client?"] == True].Value.tolist() + owner_details_edit[owner_details_edit["Ask Client?"] == True].Value.tolist()
            ...
            df_tr_edit = st.data_editor(df_tr,
                                    column_config = {
                                        "Raise Request": st.column_config.CheckboxColumn("Ask Client?", default = False)
                                    },
                                    hide_index = True,
...
def show_rd_view():
    ...
    sl, caar, att = st.tabs(["Shopping List", "CAAR", "Attestation"])
    ...
    with sl:
        ...
        with cols[0]:
            if st.button("Save Progress", key = "rd_sp", use_container_width=True):
                pass
        ...
        with cols[2]:
            if st.button("Next",use_container_width=True, key = 'rd_n' ):
                pass
    ...
    with caar:
        ...
        if st.session_state.fetch_transaction_data == True:
            ...
            st.markdown("""Here comes the summary
                        .
                        .
                        .
                        .""")
    ...
    •	Use the free software Adobe Acrobat Reader version 9.0 or above to complete this form
    •	In the case of selection fields, please select from the list
    •	Please tick boxes that apply like this ☐
...
""")
                        st.text_input("Client Name*", "ABC company", key = "vhava")
                        st.text_input("Client CIS/SDS*", "1234567", key = "vhavas")
...
def color_by_criticality(row):
    ...
    """
    if row["Issue Criticality"] == "Critical":
        return ["background-color: red"] * len(row)
    elif row["Issue Criticality"] == "Moderate":
        return ["background-color: orange"] * len(row)
    elif row["Issue Criticality"] == "Minor":
